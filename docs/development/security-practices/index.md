...
| Deploy CSP enforcement workflow, Trusted Types, or cross-origin isolation | [Browser Security Policies](browser-security-policies.md) | Deploy CSP through three phases — report-only discovery (days to weeks), analyze violations filtering extension noise, then enforce only when violations near zero. Trusted Types blocks DOM-sink XSS but requires framework/widget audit before enforcement. COEP requires every embedded subresource to serve Cross-Origin-Resource-Policy. |
