...
tldr: "Set `reporter: 'html'`, `outputDir: 'test-results'`, `forbidOnly: !!process.env.CI`, pin `colorScheme`/`locale`/`timezoneId` in `use`, and cap `workers` explicitly — its 50%-of-cores default oversubscribes a single backend container."
---
