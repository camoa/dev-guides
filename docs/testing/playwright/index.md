...
| Use ATK helpers in plain Playwright | [ATK Integration](pw-e2e-atk-integration.md) | Scope locators from ATK's body classes and data-media-id, and import atkCommands helpers without the full catalog. Against one DDEV database, restore it once per run and use one worker — a worker-scoped restore fixture isolates nothing. |
