...
| Decide which AJAX patterns NOT to migrate | [When NOT to Migrate](when-not-to-migrate.md) | Keep AJAX for ordered command sequences, CSS manipulation, jQuery UI dialogs with complex options, jQuery data API, and contrib dependencies. Native <dialog> elements can migrate to HTMX via ->on('::afterSwap', 'showModal()'). |
