...
| Choose a field pattern across N bundles (Q1–Q6 ordered ruleset) | [Storage Decision Tree](storage-decision-tree.md) | Apply this BEFORE picking a field type. Ordered Q1–Q6 ruleset commits to a pattern (custom_field / shared storage / taxonomy / wrapper bundle / entity reference / bundle-private); field-type selection then narrows within the chosen pattern. |
...
| Share one storage across N bundles (when to share, how to name) | [Shared Field Storage Strategy](shared-field-storage-strategy.md) | When the same concern recurs on ≥2 bundles, emit ONE storage + N instances with concern-shaped names (`field_eyebrow`, not `field_hero_eyebrow`). Companion to field-storage-configuration (mechanism) — this covers strategy: when to share, generic naming, cardinality reconciliation, dependency ordering. |
