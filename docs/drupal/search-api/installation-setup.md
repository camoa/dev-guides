...
> When installing Search API and creating your first server and index.
...
| 8 | Index content | Index → View tab → "Index now" or `drush sapi-i` |
...
| Route | Purpose |
|---|---|
| `/admin/config/search/search-api` | Main admin — servers and indexes |
| `/admin/config/search/search-api/add-server` | Create server |
| `/admin/config/search/search-api/add-index` | Create index |
| `/admin/config/search/search-api/index/{index}` | View index status |
| `/admin/config/search/search-api/index/{index}/fields` | Configure fields |
| `/admin/config/search/search-api/index/{index}/processors` | Configure processors |

## Pattern: Drush Commands
...
- **Not uninstalling core Search** — Running both wastes resources.
- **Forgetting to index** — After adding fields or changing processors, you must reindex.
- **Not creating a search view mode** — The default "Full content" view mode indexes too much. Create a dedicated view mode.

## See Also

- [Server Configuration](server-configuration.md) — configuring the server
- [Index Configuration](index-configuration.md) — configuring the index
