...
| Decide if my data is polymorphic — multiple mutually-exclusive sub-shapes | [Polymorphic Compound Fields](polymorphic-compound.md) | Use custom_field when data has ≥2 mutually-exclusive sub-shapes that no single core field type can express (logo as image OR text fallback; contact as email OR phone OR URL). Three-test detection rule, column shape declaration, formatter branching encodings. The canonical custom_field trigger. |
