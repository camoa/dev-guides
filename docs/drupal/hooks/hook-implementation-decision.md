...
The shape of `core/modules/system/src/Hook/PageAttachmentsHook.php`. Core's body calls `BareHtmlPageRenderer::systemPageAttachments()`, which first appears in 11.1.0, so this body attaches a library and runs on 10.1 as well:
...
final class PageAttachmentsHook {
  ...
  public function pageAttachments(array &$page): void {
    $page['#attached']['library'][] = 'my_module/my_library';
  }

}
```

On 11.1.0 and later the class is registered as an autowired service with the class name as service id, so no `services.yml` entry is needed (`core/lib/Drupal/Core/Hook/HookCollectorPass.php`, `registerHookServices()`).

The legacy shim, only when you must support core older than 11.1.0, goes in `my_module.module`. Those cores have no `HookCollectorPass` and do not register the class, so the shim's `\Drupal::service()` call needs a manual entry in `my_module.services.yml` (change record 3442349). On 11.1+ the collector leaves an existing definition alone:

```yaml
# my_module.services.yml
services:
  Drupal\my_module\Hook\PageAttachmentsHook:
    class: Drupal\my_module\Hook\PageAttachmentsHook
    autowire: true
```
...
- Keeping the shim after dropping 10.1 to 11.0.x support → dead code. The collector skips `#[LegacyHook]` functions, so the scan is unaffected, but the file still loads. In core the attribute survives only in `core/themes/stable9/stable9.theme` and two test fixtures.
