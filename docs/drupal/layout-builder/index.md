...
| Enable LB on a content type or entity view mode | [Enabling Layout Builder](enabling-lb.md) | When you want to enable Layout Builder on a content type, custom entity type, or specific view mode. |
...
| Understand section structure and layout plugins | [Sections & Layouts](sections-layouts.md) | When you need to understand how sections work, what layout plugins provide, and how regions structure content within a section. |
| Group multiple blocks in one section vs split into siblings | [Multi-Block Sections](multi-block-sections.md) | When a page region aggregates 2+ blocks that share authoring intent AND section-level styling AND coordinated order, emit ONE LB section with N SectionComponents instead of sibling sections. Three-signal detection rule, SectionComponent YAML, layout choice, counter-example for Pattern 2 multi-item collections. |
...
| Choose between default layouts and per-entity overrides | [Defaults vs Overrides](defaults-vs-overrides.md) | When deciding whether to allow per-entity layout customization or use a single default layout for all entities of a bundle. |
| Restrict which blocks editors can add | [Layout Builder Restrictions](lb-restrictions.md) | When you need to limit which blocks, layouts, or sections editors can add in Layout Builder to prevent overwhelming choices or enforce content governance. |
...
| Style LB admin UI and frontend output | [Theming Layout Builder](theming-lb.md) | When styling Layout Builder admin UI, layout frontend output, or section/component markup. |
