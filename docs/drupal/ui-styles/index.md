...
| Style UI Patterns / SDC components | [UI Patterns & SDC Integration](ui-patterns-sdc.md) | Enable ui_styles_ui_patterns; pattern instances in Layout Builder gain UI Styles form fieldsets per exposed prop slot. Styles merge into the rendered Twig template's attributes. Component-internal markup not exposed as a styleable slot cannot be targeted — modify the Twig template instead. |
