...
| Pick between UI Styles, UI Skins, UI Icons for a layout concern | [Decision Rule](decision-rule.md) | Q1/Q2/Q3 dispatch — Q1 single independent style choice → UI Styles; Q2 coordinated bundle (e.g., dark mode flips bg + text + CTA together) → UI Skin; Q3 icon-pack choice → UI Icons. Greenfield posture: adopt Styles + Icons day one; defer Skins until coordinated variants recur. |
