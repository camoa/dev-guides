...
| Decide taxonomy vs list_string vs entity reference (cross-bundle classification) | [Cross-Cutting Classification](cross-cutting-classification.md) | When the values are USED AS A CLASSIFICATION DIMENSION across MULTIPLE bundles AND the set is finite → taxonomy vocabulary + shared entity_reference field storage. Decision rule, worked examples, hierarchy considerations, sharing the field storage across bundles. Q3 entry point of the Storage Decision Tree. |
