...
> When creating a Mailgun account or adding a sending domain. The region choice affects where email content is stored and which API endpoint your Drupal site hits. Choice is not retroactive — wrong-region keys won't authenticate.
...
| **EU** | `api.eu.mailgun.net` | `smtp.eu.mailgun.org` | GDPR data-residency requires EU storage of email content/metadata. Customers in EU/EEA jurisdictions. |
...
**Setting region in drupal/mailgun**:
- Admin UI: `/admin/config/services/mailgun` → "API endpoint" dropdown → US or EU
- Programmatic via `settings.php`:
...
## Common Mistakes
- **Wrong**: Generating an API key on US dashboard but configuring Drupal to use `api.eu.mailgun.net` → **Right**: 401 Unauthorized — region mismatch is the #1 support ticket. Match endpoint to where the domain was created.
