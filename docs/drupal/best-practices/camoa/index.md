...
| Avoid unnecessary custom modules | [Avoid Unnecessary Custom Modules](avoid-unnecessary-custom-modules.md) | Apply reuse, extend, create in order — CSS/config/existing-contrib first (reuse), then Twig template override and preprocess hooks (extend), then custom module (create). Contrib is shared infrastructure; your template is yours to patch forever. |
