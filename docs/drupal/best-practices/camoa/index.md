...
| Avoid unnecessary custom modules | [Avoid Unnecessary Custom Modules](avoid-unnecessary-custom-modules.md) | A custom module is the last resort — try CSS, Twig template overrides (before preprocess hooks), config-only solutions, and existing contrib before creating custom code. Templates beat preprocess because they have no PHP runtime cost, no caching risk, and no maintenance surface. |
