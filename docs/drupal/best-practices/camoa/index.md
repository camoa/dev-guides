...
| DDEV + Composer path repo for drupal-recipe | [DDEV + Composer Path Repo for drupal-recipe Packages](ddev-composer-path-repo-drupal-recipe.md) | Path repos symlink recipe packages and the symlink dangles in the DDEV container — force `symlink: false`, use a `vcs` repo for tag-accurate tests, and revert the `version` key require writes into the source. |
