...
| Avoid unnecessary custom modules | [Avoid Unnecessary Custom Modules](avoid-unnecessary-custom-modules.md) | A custom module is the last resort — try CSS in the theme, theme preprocess hooks, config-only solutions, and existing contrib before creating custom code. Every custom module is a permanent maintenance liability and new attack surface. |
