...
**Performance: Proper cache invalidation**
```php
public function build() {
  ...
  return [
    ...
    '#cache' => [
      'tags' => array_merge(
        $config->getCacheTags(),
        Cache::mergeTags(...array_map(fn($n) => $n->getCacheTags(), $nodes))
      ),
...
- Forgetting `accessCheck(TRUE)` → Exposes content users shouldn't see
- Using `#markup` with user-provided data → XSS vulnerability if not escaped
- Long-running operations in `build()` → Blocks entire page render
- Not profiling blocks → "Feels slow" is too late; use Webprofiler or Blackfire
- Caching user-specific content without `user` context → Shows User A's data to User B
- Storing secrets in block configuration → Configuration is exportable, visible in Git
