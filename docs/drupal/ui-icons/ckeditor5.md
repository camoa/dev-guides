...
tldr: "Enable ui_icons_ckeditor5 + ui_icons_text, add the Icon toolbar button and Embed icon filter to a text format. The filter must run after filter_html, and allowed-tags needs all six drupal-icon attributes, or the format form blocks saving."
drupal_version: "11.x"
---
...
The Icon button opens a modal picker. On save, a `<drupal-icon>` tag is inserted:
...
The `icon_embed` text filter transforms the tag at render time using the pack's template. Besides the two `data-` attributes it also passes `class`, `aria-label`, `aria-hidden`, and `role` through to the rendered markup.
...
- **Wrong**: listing only some packs implicitly → **Right**: restrict via the filter's `allowed_icon_pack` setting
