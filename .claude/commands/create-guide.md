...
- **Never commit generated indexes or sources.** Do NOT stage `site/`, `llms.txt`, `llms.hash`,
  ...
  *outside* this repo (in `~/workspace/claude_memory/guides/`) and stays there; CI regenerates
  all indexes on merge. A PR touches **only** `docs/**` (including `.nav.yml` files) and `partition-manifest.json` — never `mkdocs.yml` (navigation is generated by the `awesome-nav` plugin).
...
`guide-meta:` block **and** the 3-column "I need to… | Guide | Summary" routing table),
navigation (the topic's `.nav.yml` for ordering, plus a one-line `docs/.nav.yml` entry **only**
when the category is brand-new), the parent **category** index row, and the `partition-manifest.json`
...
```bash
git add docs/<topic>/ docs/.nav.yml partition-manifest.json   # docs/.nav.yml only changes for a new category
git status --short            # confirm: ONLY docs/**, partition-manifest.json
```
